...
client.on('interactionCreate', async interaction => {
	...
	if (commandName === 'teams') {
		...
		const teamEmbed = new EmbedBuilder().setTitle(teamname).setColor(0xC70039).setThumbnail(returnInfo['team_logo'])
		.addFields(
	...
	if (commandName === 'player') {
		...
		const playerEmbed = new EmbedBuilder().setTitle(playername).setColor(0xC70039).setThumbnail(playerInfo['player_img'])
		.addFields(
	...
	if (commandName === 'roster') {
		...
		const roster = await TeamRoster(teamname);
		var rosterString = ''
		roster.forEach((player) => {
			rosterString += player + '\n'
		});
		const rosterEmbed = new EmbedBuilder().setTitle(teamname).setColor(0xC70039)
		.addFields(
	...
	if (commandName === 'teamseason') {
		...
		const teamseason = await TeamSeason(teamname, seasonyear);
		var wins = parseInt(teamseason['season_wins']);
		var losses = parseInt(teamseason['season_losses']);
		var gp = wins + losses;
		const seasonEmbed = new EmbedBuilder().setTitle(teamname).setColor(0xC70039)
		.addFields(
			{ name: 'Season', value: teamseason['season_year'] },
			{ name: 'Wins', value: teamseason['season_wins'] },
			{ name: 'Losses', value: teamseason['season_losses'] },
			{ name: 'Games Played', value: gp.toString() },
			{ name: 'W/L%', value: teamseason['season_wlp'] },
			{ name: 'Coach', value: teamseason['season_coach'] }
		);
	...
	if (commandName === 'playerseason') {
		...
		const pSeasonEmbed = new EmbedBuilder().setTitle(playername).setColor(0xC70039)
		.addFields(
	...
	if (commandName === 'playerplayoff') {
		...
		const pPlayoffEmbed = new EmbedBuilder().setTitle(playername).setColor(0xC70039)
		.addFields(
...
async function TeamInfo(teamname) {
    try {
        ...
        var Team = TeamInfo.collection(teamname).find();
...
async function TeamRoster(teamname) {
	try {
        await mongo_client.connect();
        const Team = mongo_client.db(teamname.replace(' ', ''));
...
async function Player(playername) {
	try {
		...
		await Promise.all(Teams.map(async (team) => {
			var Player = await mongo_client.db(team['name'].replaceAll(' ', '')).collection('Players').find({ player_name: playername}).toArray();
